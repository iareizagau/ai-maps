...
COSTS_SQL_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "scripts", "define_costs.sql"
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"  bikepacking_cost : {'✓ existe' if has_bikepacking else '✗ falta'}"
        )
        self.stdout.write(
            f"  hiking_cost      : {'✓ existe' if has_hiking else '✗ falta'}"
        )

        if check_only:
            if has_bikepacking and has_hiking:
                self.stdout.write(
                    self.style.SUCCESS("✓ Costes ya configurados correctamente.")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
        ...
        if has_bikepacking and has_hiking:
            self.stdout.write(
                self.style.SUCCESS("✓ Costes ya configurados. Nada que hacer.")
            )
            return

        # 3. Ejecutar el SQL
        sql_path = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "../../scripts/define_costs.sql")
        )
        if not os.path.exists(sql_path):
            self.stderr.write(
                self.style.ERROR(f"✗ No se encuentra el script: {sql_path}")
            )
            return
