...
def is_cache_stale() -> tuple[bool, Optional[float]]:
...
def classify_article(raw: RawArticle) -> Optional[dict]:
    prompt = CLASSIFY_PROMPT_TEMPLATE.format(
        title=raw.title, snippet=raw.snippet or '(sin descripción)',
    )
    try:
    ...
    try:
        ...
        log.warning("Gemini returned non-JSON for %s: %s | raw=%r", raw.source_url, e, text[:200])
        return None
...
def _embed_for_search(article: NewsArticle) -> Optional[list]:
...
def refresh(*, embed: bool = True, throttle_s: float = 0.6) -> RefreshStats:
    ...
    existing = set(
        NewsArticle.objects.filter(source_url__in=list(seen_in_batch))
...
def list_articles(*, limit: int = 60, relevance: Optional[str] = None,
...
def recent_affecting_plan(*, days: int = ALERT_WINDOW_DAYS, limit: int = 3):
    ...
    return list(
        NewsArticle.objects
        .filter(affects_user_plan=True, published_at__gte=cutoff)
        .order_by('-published_at')[:limit]
    )


def ranked_for_user(*, query_text: str, days: int = ALERT_WINDOW_DAYS,
                    limit: int = 3) -> list:
    ...
    return list(
        base.annotate(_distance=CosineDistance('embedding', qvec))
