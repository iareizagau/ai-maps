    ...
    def test_dataset_and_news_carry_distinct_headers(self):
        docs = [
            services.RetrievedDoc(
                ...
                source_type="dataset",
                score=0.9,
                content="ayudas hasta 7000€",
                kind="dataset",
            ),
            services.RetrievedDoc(
                id=2,
                title="Subvención ampliada",
...
class AnswerMergeTests(TestCase):
    ...
    def test_news_and_docs_merged_and_sorted_by_score(self, embed_mock, gen_mock):
        ...
        with (
            mock.patch.object(services, "retrieve_topk", return_value=[doc_hi, doc_lo]),
    ...
    def test_low_score_news_filtered_with_min_score(self, embed_mock, gen_mock):
        ...
        with (
            mock.patch.object(services, "retrieve_topk", return_value=[good_doc]),
            mock.patch.object(
                services, "retrieve_news_topk", return_value=[noise_news]
            ),
        ):
            result = services.answer(query="¿algo?", k=3)
