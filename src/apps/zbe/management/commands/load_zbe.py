...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(data_dir):
            self.stdout.write(
                self.style.ERROR(f"Directorio de datos no encontrado: {data_dir}")
            )
            return
        ...
        for filename in files:
            ...
            with open(file_path, encoding="utf-8") as f:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"  Error al leer {filename}: {e}")
                    )
                    continue
            ...
            for feature in data.get("features", []):
                ...
                if geom_type not in ["Polygon", "MultiPolygon"]:
                    ...
                    if geom_type != "Point":
                        self.stdout.write(
                            self.style.WARNING(
                                f"  [Saltado] Tipo de geometría {geom_type} no compatible."
                            )
                        )
                    continue
                ...
                # Detectar nombre
                name = (
                    properties.get("Izena")
                    or properties.get("zona_zbe")
                    or properties.get("Nombre")
                    or properties.get("name")
                    or properties.get("nombre")
                    or properties.get("BOUNDARY")
                )
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"  Error en fragmento de {name}: {e}")
                    )
            ...
            for name, info in zones_to_process.items():
                ...
                try:
                    ...
                    zone, created = LowEmissionZone.objects.update_or_create(
                        name=name,
                        defaults={"description": info["desc"], "geom": merged_geom},
                    )

                    status = "Creada" if created else "Actualizada (Fusionada)"
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  [{status}] {name} ({len(info['polygons'])} fragmentos)"
                        )
                    )
                    total_count += 1
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(
                            f"  Error al guardar zona fusionada {name}: {e}"
                        )
                    )

        self.stdout.write(
            self.style.SUCCESS(
                f"Proceso finalizado. Total de zonas procesadas: {total_count}"
            )
        )
