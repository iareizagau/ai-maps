...
def run_cmd(cmd, shell=True):
    ...
    result = subprocess.run(cmd, shell=shell, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", errors="ignore")
    if result.returncode != 0:
        print(f" [ERROR] ({result.returncode}): {result.stderr}")
    return result.returncode == 0, result.stdout, result.stderr

def main():
    parser = argparse.ArgumentParser(description="Importacion progresiva de carreteras en pgRouting")
    parser.add_argument("--pbf", type=str, required=True, help="Ruta al archivo PBF dentro del contenedor maps_db (ej: /tmp/switzerland-latest.osm.pbf)")
    parser.add_argument("--bbox", type=str, required=True, help="Bounding box global (min_lon,min_lat,max_lon,max_lat)")
    parser.add_argument("--step", type=float, default=0.5, help="Tamano de la tesela en grados (defecto: 0.5)")
    parser.add_argument("--clean", action="store_true", help="Vaciar las tablas antes de comenzar la importacion")
    ...
    try:
        ...
        print(" [ERROR] Formato de BBOX invalido. Debe ser: min_lon,min_lat,max_lon,max_lat")
        sys.exit(1)
    ...
    for idx, (t_min_lon, t_min_lat, t_max_lon, t_max_lat) in enumerate(tiles):
        ...
        if not ok:
            print(f" [WARN] Error al filtrar carreteras de la tesela {tile_bbox}. Saltando...")
            continue
    ...
    print("=" * 60)
    print(f" [DONE] PROCESO COMPLETADO CON EXITO!")
