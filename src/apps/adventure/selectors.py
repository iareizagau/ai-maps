...
def get_adventure_route(start_coords, end_coords, profile="bikepacking", scenic=False):
    ...
    if not features:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                f"SELECT COUNT(*) FROM pgr_ways WHERE the_geom && ST_MakeEnvelope({min_lon}, {min_lat}, {max_lon}, {max_lat}, 4326)"
            )
...
def get_vanlife_tsp_route(
    waypoints,
    pernocta_preference="both",
    max_driving_hours=3.0,
    vehicle_height=2.0,
    vehicle_width=1.9,
    scenic=False,
):
    ...
    for idx, coord in enumerate(waypoints):
        ...
        if not node:
            return {
                "error": f"No se pudo encontrar ningún nodo de carretera cercano para el punto #{idx + 1} ({coord[1]}, {coord[0]})."
            }
    ...
    if len(nodes) < 2:
        return {
            "error": "Se necesitan al menos 2 puntos de paso válidos para calcular la ruta."
        }
    ...
    if not features:
        return {
            "error": "No se pudo trazar la geometría física de los caminos para el bucle circular."
        }
    ...
    for f in features:
        ...
        if current_stage_dist >= max_driving_distance_m:
            ...
            pernoctas = PointOfInterest.objects.filter(
                poi_type__in=["camp_free", "camp_paid"]
            )
            ...
            closest_options = sorted(pernoctas, key=lambda p: p.location.distance(pnt))[
                :3
            ]
            options_list = []
