...
from apps.gailur.models import Crag, Route, Sector
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        feeds = [
            {
                "url": "https://eskalatzencas.blogspot.com/feeds/posts/default?max-results=500",
                "lang": "es",
            },
            {
                "url": "https://eskalatzeneus.blogspot.com/feeds/posts/default?max-results=500",
                "lang": "eu",
            },
        ]
        ...
        for feed in feeds:
            ...
            for entry in root.findall("atom:entry", ns):
                ...
                route_slug = slugify(url_path.replace(".html", "").replace("/", "-"))[
                    :255
                ]
                ...
                match = re.match(
                    r"^(?P<zone>.*?) - (?P<route>.*?) \((?P<specs>.*)\)$", title
                )
                ...
                if match:
                    ...
                    crag, created_crag = Crag.objects.get_or_create(
                        slug=zone_slug, defaults={"location": Point(0, 0)}
                    )

                    # Update crag name and description based on current language
                    if lang == "es":
                        crag.name_es = zone_name
                        crag.description_es = "Zona extraída del blog Eskalatzencas."
                    else:
                        crag.name_eu = zone_name
                        crag.description_eu = "Eskalatzeneus blogetik ateratako zona."
                    crag.save()

                    # 2. Sector
                    sector, _ = Sector.objects.get_or_create(
                        crag=crag, name="Principal"
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Finished {lang.upper()}. New: {new_count}, Updated with {lang.upper()}: {update_count}"
                )
            )
