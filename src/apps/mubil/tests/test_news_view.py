...
def _make(url, *, title="Titular", relevance="ESPANA", tags=None, days_ago=0):
    pub = datetime(2026, 6, 5, tzinfo=UTC)
...
class NewsPageTests(TestCase):
    ...
    def test_tag_filter(self):
        _make(
            "https://example.com/sub", title="Con tag subvención", tags=["subvencion"]
        )
        _make(
            "https://example.com/luz", title="Con tag precio_luz", tags=["precio_luz"]
        )
        with mock.patch("apps.mubil.views.refresh_news.delay"):
            resp = self.client.get(
    ...
    def test_celery_down_does_not_500(self):
        ...
        with mock.patch(
            "apps.mubil.views.refresh_news.delay",
