...
def tonight_view(request):
    ...
    context = {
        ...
        "page_h1": _("%d eventos para bailar hoy") % count
        if count > 0
        else _("SBK esta noche"),
        "meta_description": _(
            "¿Dónde bailar esta noche? Lista actualizada de sesiones sociales, talleres y fiestas SBK para hoy."
        ),
        "OPENWEATHERMAP_API_KEY": settings.OPENWEATHERMAP_API_KEY,
    }
    return render(request, "sbk/pages/home/map.html", context)
...
def type_view(request, type_slug):
    ...
    type_map = {
        "festivales": (
            "festival",
            _("Próximos Festivales SBK"),
            _(
                "Calendario de congresos y festivales internacionales de Bachata, Salsa y Kizomba."
            ),
        ),
        "socials": (
            "party",
            _("Sesiones Sociales y Fiestas SBK"),
            _("Encuentra los mejores sociales para bailar durante la semana."),
        ),
        "talleres": (
            "workshop",
            _("Talleres y Bootcamps SBK"),
            _("Mejora tu estilo con los mejores talleres intensivos."),
        ),
    }
...
def city_view(request, city_slug):
    ...
    context = {
        ...
        "meta_description": _(
            "Descubre todos los eventos de baile en %s. Festivales, sociales y academias en un solo mapa."
        )
        % city_name,
...
def practice_view(request):
    ...
    context = {
        ...
        "meta_description": _(
            "Conecta con otros bailadores. Encuentra pareja para talleres o compañeros para compartir alojamiento."
        ),
    }
...
def person_directory_view(request):
    ...
    context = {
        ...
        "meta_description": _(
            "Conoce a los mejores DJs, profesores y artistas de la escena SBK internacional."
        ),
    }
...
def person_detail_view(request, slug):
    ...
    context = {
        ...
        "page_h1": person.name,
        "meta_description": person.bio[:160]
        if person.bio
        else _("Perfil oficial de %s en SBK Hub.") % person.name,
    }
...
def events_json(request):
    ...
    events = (
        Event.objects.prefetch_related("reviews", "checkins", "vibe_reports")
        .filter(moderation_status__in=["pending", "verified"])
        .filter(
    ...
    for e in events:
        ...
        avg_rating = (
            sum(r.overall_rating for r in reviews) / len(reviews) if reviews else None
        )
        ...
        data.append(
            {
                ...
                "user_status": user_statuses.get(e.id),
                "avg_rating": round(avg_rating, 1) if avg_rating else None,
                "review_count": len(reviews),
                "pulse": {"active_checkins": active_checkins, "vibe": vibe_summary},
            }
        )
...
def toggle_event_status(request, event_id):
    ...
    if (
        target_status not in [s[0] for s in UserEventStatus.choices]
    ...
    user_event, created = UserEvent.objects.update_or_create(
        user=request.user, event=event, defaults={"status": target_status}
    )

    if event.is_user_submitted and not event.is_verified:
        going_count = UserEvent.objects.filter(
            event=event, status=UserEventStatus.GOING
        ).count()
        if going_count >= 3:
    ...
    return JsonResponse({"status": user_event.status, "is_verified": event.is_verified})
...
def passport_view(request):
    ...
    """
    user_events = (
        UserEvent.objects.filter(user=request.user)
    ...
    going_events = [
        ...
        and ue.event.start_date
        and ue.event.start_date >= now
    ]
    interested_events = [
        ue
        for ue in user_events
        if ue.status == UserEventStatus.INTERESTED
        and ue.event.start_date
        and ue.event.start_date >= now
    ]
    past_events = [
        ue for ue in user_events if ue.event.start_date and ue.event.start_date < now
    ]
    ...
    context = {
        ...
        "stats_countries": len(
            set([ue.event.country for ue in user_events if ue.event.country])
        ),
...
def event_community_api(request, event_id):
        ...
        community.append(
            {
                ...
                "avatar_url": getattr(ue.user, "avatar_url", None)
                or f"https://ui-avatars.com/api/?name={ue.user.username}&background=random",
    ...
    notices_qs = (
        EventNotice.objects.filter(event=event)
        .select_related("user")
        .order_by("-created_at")
    )
    notices = []
        ...
        notices.append(
            {
                ...
                "avatar_url": getattr(n.user, "avatar_url", None)
                or f"https://ui-avatars.com/api/?name={n.user.username}&background=random",
            }
        )

    return JsonResponse(
        {"event_name": event.name, "community": community, "notices": notices}
    )
...
def submit_event_review(request, event_id):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "success", "message": "Reseña guardada correctamente"}
        )
...
def submit_local_social(request):
    ...
    try:
        ...
        person_profile = Person.objects.filter(
            claimed_by=request.user, is_verified=True
        ).first()
        ...
        return JsonResponse(
            {
                "status": "success",
                "message": "¡Social añadido con éxito!",
...
def submit_event_notice(request, event_id):
    ...
    try:
        ...
        notice = EventNotice.objects.create(
            event=event, user=request.user, category=category, message=message[:250]
        )
        add_xp(request.user, 10)
        return JsonResponse(
            {"status": "success", "message": "Anuncio publicado correctamente"}
        )
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=400)
...
def check_in(request, event_id):
    ...
    if created:
        ...
        checkin.save()
        return JsonResponse(
            {"status": "success", "message": "Has refrescado tu presencia"}
        )
...
def submit_vibe_report(request, event_id):
    ...
    if event.start_date:
        ...
        if now < start_margin or now > end_bound:
            return JsonResponse(
                {"error": "Solo puedes reportar el ambiente durante el evento."},
                status=400,
            )
...
def dance_venue_claim(request, venue_id):
    ...
    existing = DanceVenueClaim.objects.filter(
        venue=venue, claimant=request.user
    ).first()
    ...
    if request.method == "POST":
        ...
        form = DanceVenueClaimForm(
            instance=existing, initial={"contact_email": request.user.email}
        )
