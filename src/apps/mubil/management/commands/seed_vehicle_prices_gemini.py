...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        qs = (
            Vehicle.objects.filter(price_source=Vehicle.PriceSource.HEURISTIC)
        ...
        if not candidates:
            self.stdout.write(
                self.style.WARNING(
                    "Sin candidatos. ¿Has corrido la Capa 2 (heurística)?"
                )
            )
            return
        ...
        for i, v in enumerate(candidates, start=1):
            ...
            if not gemini_price_lookup.validate_against_heuristic(
                gemini_price=est.price_eur,
                heuristic_price=heur,
            ):
                rejected_outlier += 1
                self.stdout.write(
                    self.style.WARNING(
