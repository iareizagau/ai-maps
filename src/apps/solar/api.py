...
from .services import estimate_peak_power, get_pvgis_data
            ...
            features.append(
                {
                    "type": "Feature",
                    "id": b.osm_id,
                    "geometry": {"type": "Polygon", "coordinates": b.geom.coords},
                    "properties": {
                        ...
                        "color": "#fbbf24"
                        if has_solar
                        else "#ffffff",  # Dorado para analizados, blanco para el resto
...
def get_building_at(request, lat: float, lon: float):
    ...
    building = (
        Building.objects.filter(geom__intersects=point)
        .select_related("potential")
        .first()
    )
    ...
    if not building:
        building = (
            Building.objects.annotate(distance=Distance("geom", point))
            .filter(geom__distance_lte=(point, D(m=15)))
            .select_related("potential")
            .order_by("distance")
            .first()
        )
    ...
    if hasattr(building, "potential"):
        ...
        potential_data = {
            ...
            "kpis": {
                "co2_offset": round(
                    potential.annual_generation_kwh * 0.25
                ),  # 0.25kg/kWh approx
                "annual_savings": round(
                    potential.annual_generation_kwh * 0.18
                ),  # 0.18€/kWh approx
