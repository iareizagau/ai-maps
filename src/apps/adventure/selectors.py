...
def get_adventure_route(start_coords, end_coords, profile='bikepacking'):
    ...
    # Nota: Usamos la tabla pgr_ways creada por osm2pgrouting
    query = f"""
        SELECT 
        ...
        FROM pgr_dijkstra(
            'SELECT gid as id, source, target, {cost_column} as cost FROM pgr_ways',
            %s, %s, directed := false
        ) AS di
    ...
    return {
        ...
        "metadata": {
            ...
            "elevation_gain": 0,  # A rellenar con datos reales
            "elevation_loss": 0   # A rellenar con datos reales
        }
    }
